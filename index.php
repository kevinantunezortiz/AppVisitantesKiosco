...
<body>
  ...
  <div class="row">
    ...
    <div class="col-2">
      <button class="btn btn-lg btn-outline-primary" id="btnGrupos">
        Registrar Grupos
      </button>
    </div>
  </div>
  ...
  <div class="container mt-3 pb-5">
    <div id="step-1" class="card card-custom p-4">
      <h4 id="titulo-persona">Registro Visitante</h4>
      <form id="form-registro">
        <h6>Paso 1</h6>
        <p class="fw-bold mb-2">
          ¿Cómo te identificas? (Elige una de las 15 Opciones)
        </p>
        <div class="row mb-3">
          <script>
            const grupos = [{
    ...
    <div class="container">
      <div
        ...
        <div>
          ...
          <button id="btn-close-admin" class="btn btn-outline-dark btn-sm">
            Cerrar
          </button>
        </div>
      </div>
  ...
  <div class="container">
    <div id="grupo-panel" class="section card card-custom">
      <div class="row align-items-center">
        <div class="col m-1">
          <h2>Registro de Grupo de Visitantes</h2>
        </div>
        <div class="col"><button id="btn-close-grupos" class="btn btn-outline-primary">Cerrar</button></div>
        <?php
        require('grupos.php')
        ?>
      </div>
  ...
  <script type="module">
    ...
    function updateUI() {
      if (actualP >= totalP) {
        $("#btn-siguiente").hide();
        $("#btn-finalizar").removeClass("d-none").show();
      }
    }
    ...
    $("#btn-admin-logo").on("dblclick", async function() {
      $("#admin-panel").fadeIn();
      await cargarFiltros();
    });
    ...
    async function cargarFiltros() {
      ...
      $("#filter-year").empty();
      anios.forEach((a) =>
        $("#filter-year").append(`<option value="${a}">${a}</option>`),
      );
      $("#filter-month").empty();
      meses.forEach((m) =>
        $("#filter-month").append(
    ...
    $("#btn-generar-reporte").click(async function() {
      ...
      snap2.forEach((doc)=>{
        const d = doc.data();
        if (d.anio === selYear && d.mes === selMonth) {
          counts["infancias_adolescentes"] = counts["infancias_adolescentes"] + d.infancias_adolescentes_total;
          counts["personas_jovenes"] = counts["personas_jovenes"] + d.jovenes_total;
          counts["adultosmayores"] = counts["adultosmayores"] + d.adultos_mayores_total;
        }
      })
      let  html = `<table id="tabla-reporte" class="table table-bordered table-striped mt-3">
                        <thead class="table-dark"><tr><th>Grupo</th><th>Total</th></tr></thead><tbody>`;
      for (const k in counts) {
        html += `<td>${k}</td><td>${counts[k]}</td></tr>`;
      }
    ...
    $("#btn-close-admin").click(() => {
      $("#reporte-content").empty();
      $("#admin-panel").fadeOut();
    });
    $("#btn-export-excel").click(() => {
      XLSX.writeFile(
        XLSX.utils.table_to_book(document.getElementById("tabla-reporte")),
        "Reporte_Visitantes.xlsx",
      );
    });
